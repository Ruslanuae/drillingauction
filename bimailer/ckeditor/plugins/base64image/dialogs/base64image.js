/*
Вручную допиливал вставку data-name в код. НЕ МЕНЯТЬ!!!!!!!!!!!!!
 */
CKEDITOR.dialog.add("base64imageDialog", function(editor){
	...
	function imagePreviewLoad(s) {
		...
		imgPreview.getElement().setHtml("Загрузка...");
		...
		i.onload = function() {
			...
			/* Вставить изображение предварительного просмотра */
			try {
	...
	/* Изменение входных значений и изображения предварительного просмотра */
	function imagePreview(src){
		...
		if(src == "base64") {
			...
			name=urlI.getValue().substring(urlI.getValue().lastIndexOf('/')+1,urlI.getValue().length);
			
			
		} else if(fsupport) {
			
			/* Флажок Проверяемый файл изображения */
			if(urlCB) urlCB.setValue(false, true);
			if(fileCB) fileCB.setValue(true, true);
			
			/* Просмотр файлов и загрузка */
			...
			if(n && "files" in n && n.files && n.files.length > 0 && n.files[0]) {
				...
				/*Потому что в sessionproj.php preg_match_all не может обработать картинки выше 700кб*/
				if(n.files[0].size>700000){
					imgPreview.getElement().setHtml('"<strong style="color:#CC0000">Загрузка картинки</strong>" с компьютера более чем 700Кб не доступна.<br>Воспользуйтесь вставкой картинок через "<strong style="color:#00CC00">Ссылку<strong>"'); return;
				}
	...
	if(fsupport) {
		...
		/* Диалог с файлом и источником изображения URL-адреса */
		var sourceElements = [
			{
				type: "hbox",
				widths: ["70px"],
				...
				children: [
					{
						...
						html: 'Вставляйте картинки через поле "<strong>Ссылка</strong>", старайтесь избегать "<strong>Загрузку картинки</strong>" с компьютера. <br>Это значительно ускоряет загрузку проекта при редактировании и формирование письма при рассылке.'
					}
				]
			},
			//------------------------------------
				...
				children: [
					{
						...
						label: "Загрузка картинки:"//editor.lang.common.upload+":"
					},
					{
						type: "file",
						id: "file",
    ...
    return {
		...
		onOk : function(){
			...
			newImg.setAttribute("data-name", encodeURIComponent(name));
			src = null;
        ...
        contents: [
            {
                id: "tab-source",
                label: editor.lang.common.generalTab,//ОСНОВНОЕ
                elements: sourceElements
            },
            {
                id: "tab-properties",
                label: editor.lang.common.advancedTab,//Дополнительное
                elements: [
                    {
                        type: "text",
                        id: "alt",
