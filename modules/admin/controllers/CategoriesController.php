...
class CategoriesController extends Controller
{
    ...
    public function actionFilter()
    {
        if ($_GET['action'] == 'set') {
            ...
            $FilterCategory = FilterCategory::findOne(['name' => $Categories->name]);
            if (!$FilterCategory) {
                $FilterCategory = new FilterCategory();
                $FilterCategory->name = $Categories->name;
                $FilterCategory->parent_id = 10;
                $FilterCategory->alias = str_replace(' ', '-', strtolower($Categories->name));
                $FilterCategory->intString = '1';
                $FilterCategory->position = 0;
                if (!$FilterCategory->save()) {
                    dd($FilterCategory->getErrors());
                }
            }
